...
public class Problem1_nonOpti {
	private static HashMap<Integer, List<GraphNode>> graph;
	private static HashMap<String, Long> srcDetnMap;
	...
	public static void main(String[] args) {
		...
		srcDetnMap = new HashMap<String, Long>();
	...
	private static long getMinCost(int src, int dstn) {
		...
		for (int i = 0; i < midHopLists.size(); i++) {
			int destinationVertex = midHopLists.get(i).getDestinationVertex();
			String sdstn1 = src+":"+destinationVertex;
			if (srcDetnMap.containsKey(sdstn1)) {
				value = srcDetnMap.get(sdstn1);
			}else
				value = midHopLists.get(i).getWeight();
			minValue = value + Math.min(minValue, getMinCost(destinationVertex, dstn));
